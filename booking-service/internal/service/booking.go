...
func (s *bookingService) CancelBooking(ctx context.Context, bookingID string) error {
	...
	// Release tickets back to event-service
	log.Printf("CancelBooking: Releasing %d tickets for event %s", booking.TicketCount, booking.EventID)
	if err := s.eventClient.ReleaseTickets(ctx, booking.EventID, booking.TicketCount); err != nil {
		log.Printf("CancelBooking: ReleaseTickets error: %v", err)
		return err
	}
	log.Printf("CancelBooking: Tickets released successfully")
